...
export const mergeStylesWithUniqKeys = (...args) => args.reduce((acc, v) => merge(acc, uniqifyKeys(v)), {});

export const isTemplate = test(/{!([^}]+)}/g);
export const evalTemplate = (string, data) => is('String', string) ? string.replace(/{!([^}]+)}/g, (_, key) => pathOr(`{!${key}}`, key, data)) : string;

export const arrToObj = arr => reduce((accumulated, value, key) => attach(key, value, accumulated), {}, arr);
...
export const whenisNonZeroNumber = curryN(2, (fn, input) => when(isNonZeroNumber, defaultTo(identity, fn))(input));

export const appendUnit = unit => whenisNonZeroNumber(pipe(toString, appendString(unit)));
...
const getThemeFallback = fallBackObj => (attr, fallback) => pathOr(fallback)(attr)(fallBackObj);

export const getThemeAttrFB = fallBackObj => (attr = '', defaultTo = '') => pathOr(getThemeFallback(fallBackObj)(attr, defaultTo), concat('theme.', attr));

export const pathWithFallback = fallBackObj => (attr = '', defaultTo = '') => pathOr(getThemeFallback(fallBackObj)(attr, defaultTo), attr);
...
export const mapObjOf = curryN(2, (key, val) => pipe(toArray, map(x => objOf(x, val)), mergeAllDeepRight)(key));
...
export const reduceWhileFalsy = curryN(2, (handlerFn, list) => reduceWhile(not, handlerFn, false, list));
...
export const iterateUntilResult = computeFn => obj => reduceWhile(isUndefinedOrFalse, computeFn, false, obj);

export const whenFunctionCallWith = (...argsToGive) => when(isFunction, fnItem => fnItem(...argsToGive));
