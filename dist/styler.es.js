import { curry, pipe, isNil, complement, flip, toString, test, identity, mapValues, curryN, simplyEquals, reduce, split, reduceWhile, either, concat, toPairs, filter, divide, isEmpty, is, mergeAll, reduceRight, mergeDeepLeft, mergeDeepRight, when, toArray, both, pathOr, objOf, defaultTo, attach, path, merge, always, keys, prop, ifElse, values, pick, last, has } from '@roseys/futils';
...
var arrToObj = function arrToObj(arr) {
  return reduce(function (accumulated, value, key) {
...
var pxTo = curry(function (divisor, unit, num) {
  return whenisNonZeroNumber(pipe(divideBy(defaultTo(16, divisor)), toString, appendString(unit)), num);
});
...
var isNilOrEmptyOrFalse = either(isNilOrEmpty, simplyEquals(false));
...
var getThemeFallback = function getThemeFallback(fallBackObj) {
  return function (attr, fallback) {
    return pathOr(fallback)(split('.', attr))(fallBackObj);
  };
};
...
var isNegative = test(/^-.+/);
var lookUpValue = curryN(3, function (themeKey, val, props) {
...
var mapObjOf = curry(function (key, val) {
  return pipe(toArray, mapValues(function (x) {
    return objOf(x, val);
  }), mergeAll)(key);
...
var UnflattenObj = pipe(toPairs, mapValues(function (_ref) {
...
var includes = curry(function (comparator, value) {
  return value.includes(comparator);
});
var isObjectLiteral = is('Object');
var isSymbol = is('Symbol');
var isMap = is('Map');
var isDefined = complement(isNil);
var isNotDefined = isNil;
var isUndefinedOrFalse = either(isNotDefined, simplyEquals(false));
...
var fallbackTo = function fallbackTo(fallback) {
  return pipe(falseToNull, defaultTo(fallback));
};
...
var computeOptions = (function (_ref) {
  ...
  if (options && val) {
    ...
    if (getter) {
      val = pipe(lookupDefaultOptions(props)('functions'), whenFunctionCallWith(val, props))(getter);
    }
  }
...
var parseInlinePattern = (function (_ref) {
  ...
  var intersectedMatchers = keys(pick(keys(matchers), props));
  var matchedPropName;
  ...
  var matchedProp = prop(matchedPropName, props);
  var nonResponisiveComputedValue = computedValue;
  var isResponsiveBoolean = isString(computedValue) && is('Array', matchedProp) || is('Object', matchedProp);
  ...
  if (isObjectLiteral(computedValue) || isArray(computedValue)) {
    ...
    if (isArray(breakpoints)) {
      ...
      if (isObjectLiteral(themeBPs)) {
        themeBPs = arrToObj(values(themeBPs)); // console.log(themeBPs)
      }
    }

    var getBp = flip(prop)(themeBPs);
    ...
    var CSSObj = Object.keys(breakpoints).reduce(function (acc, bpKey) {
      ...
      var res = isNil(computeOpt(currentVal)) ? {} : bpKey === 'mobile' || bpKey === '0' || minWidth < 1.1 ? objOf(key, computeOpt(currentVal)) : objOf(["@media screen and (min-width:".concat(minWidth, ")"), key], computeOpt(currentVal));
...
var parseRules = function parseRules(parseNested, selector, value, parents, location, props, options) {
  ...
  if (isPatternBlock(selector)) {
    var res = flow(value, reduce(function (accumulated, rulesForProp, propName) {
      return flow(props, ifElse(has(propName), pipe(always(rulesForProp), whenFunctionCallWith(props[propName], props), mergeDeepRight(accumulated)), always(accumulated)));
    }, {}));
